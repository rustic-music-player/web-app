    ...
    ngOnInit() {
        this.activatedRoute.queryParams
            .pipe(
                ...
                map((path: string) => path.split("/").filter(p => p.trim().length > 0))
            )
    ...
    private getProvider(): Observable<ProviderModel> {
        return this.activatedRoute.params
            .pipe(
                ...
                switchMap(provider => this.store.select(selectProviders).pipe(map(providers => providers.find(p => p.provider === provider))))
            );
    }
        ...
        return combineLatest([this.path$, this.provider$])
            .pipe(
                ...
                switchMap(([path, provider]) => {
                    ...
                    return this.providerService.navigate(provider.provider, path.join('/'));
                }));
    }
