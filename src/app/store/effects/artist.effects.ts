...
export class ArtistEffects {
    @Effect() fetch$ = this.actions$.pipe(
        ...
        switchMap(() =>
            this.libraryApi.getArtists().pipe(
    ...
    @Effect() open$ = this.actions$.pipe(
        ...
        filter(
            ({ payload }: RouterNavigationAction<RouterState>) =>
                'artist_cursor' in payload.routerState.params
        ),
        map(
            ({ payload }: RouterNavigationAction<RouterState>) =>
                payload.routerState.params.artist_cursor
        ),
    ...
    @Effect() fetchSingle$ = this.actions$.pipe(
        ...
        switchMap((cursor) =>
            this.libraryApi.getArtist(cursor).pipe(
