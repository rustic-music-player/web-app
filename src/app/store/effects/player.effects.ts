...
export class PlayerEffects {
    ...
    @Effect({ dispatch: false }) changeVolume$ = this.actions$.pipe(
        ...
        switchMap((action: ChangePlayerVolume) => this.getPlayerId().pipe(map(id => ({ id, volume: action.payload })))),
    ...
    @Effect() playerState$ = this.socket.ws$.pipe(
        ...
        map(({ payload, player_cursor }) => new PlayerStateUpdated(player_cursor, payload))
    );

    @Effect() currentTrack$ = this.socket.ws$.pipe(
        filter(({ type }) => type === Messages.CurrentlyPlayingChanged),
        map(({ payload, player_cursor }) => new PlayerCurrentTrackUpdated(player_cursor, payload))
    );

    @Effect()
    fetchPlayers = this.playerApi.getPlayers()
    ...
    private getPlayerId = () => this.store.pipe(select(s => s.player.currentPlayer), first());
}
