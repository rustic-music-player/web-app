    ...
    switch (action.type) {
        case ProviderActionTypes.FetchSuccess:
            return {
                ...
                selected: action.payload.providers.map(
                    (provider) => provider.provider
                ),
            };
        case ProviderActionTypes.ToggleSelection:
            const index = state.selected.findIndex(
                (provider) => provider === action.payload.provider
            );
            let selected;
            if (index !== -1) {
...
export const selectProviders = (state: State) =>
